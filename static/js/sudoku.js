...
// check duplicate number in column for final check
const isColSafeCheck = (grid, r, col, value) => {
    for (let row = 0; row < CONSTANT.GRID_SIZE; row++) {
        if (r !== row && grid[row][col] === value) return false;
    }
    return true;
}

// check duplicate number in row for final check
const isRowSafeCheck = (grid, row, c, value) => {
    for (let col = 0; col < CONSTANT.GRID_SIZE; col++) {
        if (col !== c && grid[row][col] === value) return false;
    }
    return true;
}

// check duplicate number in 3x3 box for final check
const isBoxSafeCheck = (grid, r, c, box_row, box_col, value) => {
    for (let row = 0; row < CONSTANT.BOX_SIZE; row++) {
        for (let col = 0; col < CONSTANT.BOX_SIZE; col++) {
            if (((row + box_row !== r) && (col + box_col !== c )) && grid[row + box_row][col + box_col] === value) return false;
        }
    }
    return true;
}

// check in row, col and 3x3 box for final check
const isSafeCheck = (grid, row, col, value) => {
    return isColSafeCheck(grid, row, col, value) && isRowSafeCheck(grid, row, col, value) && isBoxSafeCheck(grid, row, col, row - row%3, col - col%3, value);
}

// sudoku check function for final check
const sudokuCheck = (grid) => {
    ...
    for(let i=0;i<CONSTANT.GRID_SIZE;i++)
    {
        ...
        {
            if(!isSafeCheck(grid,i,j,grid[i][j]))
            return false;
        } 
    }
